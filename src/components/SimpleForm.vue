      ...
      <div class="form">
        ...
        <div class="form-buttons">
          ...
          >
            <div class="button-group col-md-6">
              ...
              <input
                id="buttonurl"
                v-model="button.image"
                :name="`formButtons[${index}][image]`"
                type="text"
                class="form-control"
        ...
        <div class="sitebuttons">
          <div class="button-group">
            <button @click="addButton" type="button" class="btn btn-secondary">
              Add button
            </button>
          </div>
        </div>
  ...
  methods: {
    ...
    createConfig() {
      ...
      for (var i = 0; i < this.formButtons.length; i++) {
        ...
        var fullButtonAction = JSON.stringify(getButtonAction);
        var fullButtonText = JSON.stringify(getButtonText);
        var fullButtonIMG = JSON.stringify(getButtonImg);

        if (getButtonType.indexOf("command") > -1) {
          action = {
            [getButtonType]: [fullButtonAction],
          };
        } else {
          action = {
            [getButtonType]: fullButtonAction,

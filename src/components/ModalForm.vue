<template>
  <div id="app" class="container">
        ...
        <div class="formcontent">
          ...
          <div>
            ...
            <input
              id="button1Action"
              type="text"
              v-model="button1action"
              placeholder="Button 1 Action"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              id="button2Action"
              type="text"
              v-model="button2action"
              placeholder="Button 2 Action"
            />
          </div>
        </div>
      </div>
    ...
    <div class="button-group">
      <button @click="createConfig" type="button" class="btn btn-secondary">
        Generate Config
      </button>
    </div>
...
export default {
  ...
  methods: {
    createConfig() {
      ...
      var fullButtonAction1 = JSON.stringify(button1Action);
      var fullButtonText1 = JSON.stringify(this.button1text);
      var fullButtonAction2 = JSON.stringify(button2Action);
      var fullButtonText2 = JSON.stringify(this.button2text);

      // button action 1
      if (actiontype1.indexOf("command") > -1) {
        action1 = {
          [actiontype1]: [fullButtonAction1],
        };
      } else {
        action1 = {
          [actiontype1]: fullButtonAction1,
      ...
      var button1 = [
        {
          text: fullButtonText1,
          actions: action1,
        },
      ];
      var button2 = [
        {
          text: fullButtonText2,
          actions: action2,
        },
      ];

      var fullFormTitle = JSON.stringify(this.formtitle);
      var fullFormContent = JSON.stringify(this.formcontent);
